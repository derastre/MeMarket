...
public class purchaseHistory extends BaseActivity {
    ...
    private void readPurchasesHistoryFromFirebase() {
        ...
        DatabaseReference myHistoryRef = database.getReference().child("purchases").child(productId);
    ...
    private void allDataRead() {
        ...
        TextView textView = (TextView) findViewById(R.id.purchase_history_text);
        textView.setText(productId + "\n");
        for (int i = 0; i < companiesNameList.size(); i++) {
            textView.setText(textView.getText() + timeStamp.get(i) + "\n" + companiesNameList.get(i) + " " +
